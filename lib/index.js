...
import {CertificateAuthority, DEFAULT_CERT_OPTIONS, getCAoptions, getIssueOptions} from '@cto.af/ca';
import {classNames, log, normalizeArgs} from './utils.js';
...
export class MockTLSServer extends net.Server {
   ...
   *   import('node:tls').KeyObject> | undefined}
   */
  #key;
  ...
  constructor(options, secureConnectionListener) {
    ...
    super(socket_opts, sock => {
      ...
      tsock.emit = (eventName, ...args) => {
        log('tls', eventName, ...classNames(args));
        // @ts-ignore
    ...
    if (opts.cert) {
      ...
      const caOpts = getCAoptions(cert_opts);
      caOpts.temp = true;
      const authority =
        local_opts.authority || new CertificateAuthority(caOpts);
      const issueOpts = getIssueOptions(cert_opts);
      issueOpts.temp = true;
      const serverProps = authority.issueNew(issueOpts);
      this.cert = serverProps.cert;
      this.ca = serverProps.ca?.cert;
