import {classNames, extendedError, log, normalizeArgs} from './utils.js';
import {EventEmitter} from 'node:events';
import {MockSocket} from './socket.js';
import {globalKernel} from './kernel.js';

/**
 * @typedef {import('node:stream').Duplex} DuplexStream
 */
 ...
 *   ends.
 * @property {SocketCallback} [connectListener]
 */
 ...
 * @property {() => void} listeningListener If it exists, installed as
 ...
 *
 * @param {number |
 *   (SocketConnectOpts & import('node:stream').DuplexOptions)} options The
 *   server port to connect to.  Must already be listening.
 * @param {SocketCallback} [connectListener] Event listener for the 'connect'
 *   event.
 * @returns {DuplexStream} The client side HalfSocket.
...
export function getServer(options) {
  const {port} = normalizeArgs(options);
  return /** @type {Server} */ (globalKernel.lookup(port));
}

/**
 * @typedef {(socket: DuplexStream) => void} SocketCallback
 */
...
export class Server extends EventEmitter {
   ...
   */
  emit(eventName, ...args) {
    log(this.constructor.name, eventName, ...classNames(args));
   ...
   * @param {() => void} [listeningListener] If it exists, installed as
   *   a listener for the 'listening' event.
   * @returns {Server} This.
   */
  ...
  getConnections(cb) {
    ...
    process.nextTick(() => {
      if (this.listening) {
        cb(undefined, this.clients.size);
      } else {
